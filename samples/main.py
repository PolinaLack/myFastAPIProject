...
@app.post(path="/tasks")
def post_new_task(user_task: TaskCreate) -> dict[int, Task]:
    temp = Task(id=max(users_tasks) + 1, **user_task.model_dump())
    users_tasks[temp.id] = temp
    return users_tasks


@app.delete(path="/tasks/{task_id}")
def delete_task(task_id: Annotated[int, Path()]) -> dict:
...
def update_task(
    task_id: Annotated[int, Path()], user_task: TaskCreate) -> dict[int, Task]:
    users_tasks[task_id] = Task(id=task_id, **user_task.model_dump())
    return users_tasks


# @app.patch(path="/tasks/{task_id}")
# def update_part_task(
#     task_id: Annotated[int, Path()], user_task: Annotated[TaskPatch, None]) -> dict[int, Task]:
